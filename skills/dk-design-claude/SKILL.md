...
For a consequential Explore or Reimagine task, read [references/claude-surfaces.md](references/claude-surfaces.md). When subagents are available and parallel exploration materially improves quality, run the read-only direction cell defined there. Give every explorer the same product truth and content; do not reveal the preferred answer. The main agent alone synthesizes the three final concepts and owns every write.
